...
export default function Product({ data }: { data: ProductType }) {
  ...
  return (
    <div className="z-0 rounded-[12px] rounded-br-[28px] rounded-tl-[28px] bg-gradient-to-r from-[#9733ED] via-[#F22B9C] to-[#FD7A36] p-[2px] transition duration-300 ease-in-out hover:scale-[1.02] hover:shadow-xl">
      <div
        onClick={() => router.push(`/product/${data.id}`)}
        className="z-10 h-fit w-48 overflow-hidden rounded-[10px] rounded-br-[26px] rounded-tl-[26px] bg-background text-foreground/90 transition duration-300 ease-in-out hover:cursor-pointer hover:bg-background/80 hover:text-foreground sm:w-full"
      >
